...
static struct Nuitka_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    ...
    int length = package_name_end - name;
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyObject *module, PyObject *module_name, char const *name) {
    ...
    if (installed_extension_modules != NULL) {
        ...
        if (extension_module_filename != NULL) {
            ...
            return callIntoShlibModule(name, extension_module_filename_str);
        }
...
#ifndef _NUITKA_STANDALONE
static PyObject *getImportLibBootstrapExternalModule() {
    static PyObject *importlib = NULL;
    if (importlib == NULL) {
        importlib = PyImport_ImportModule("importlib._bootstrap_external");
    }
...
static PyObject *createModuleSpecViaPathFinder(PyObject *module_name, char const *parent_module_name) {
    ...
    PyObject *importlib_module_external = getImportLibBootstrapExternalModule();

    if (unlikely(importlib_module_external == NULL)) {
        return NULL;
    }

    static PyObject *source_file_loader = NULL;
    if (source_file_loader == NULL) {
        source_file_loader = PyObject_GetAttrString(importlib_module_external, "SourceFileLoader");
    }

    if (unlikely(source_file_loader == NULL)) {
        return NULL;
    }

#if 0
// TODO: Might have to put this to use too.

    static PyObject *bytecode_file_loader = NULL;
    if (bytecode_file_loader == NULL) {
        bytecode_file_loader = PyObject_GetAttrString(importlib_module_external, "SourcelessFileLoader");
    }

    if (unlikely(bytecode_file_loader == NULL)) {
        return NULL;
    }
#endif
